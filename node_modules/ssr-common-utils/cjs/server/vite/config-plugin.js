...
exports.writeEmitter = exports.asyncOptimizeChunkPlugin = exports.commonConfig = exports.rollupOutputOptions = exports.manifestPlugin = exports.chunkNamePlugin = void 0;
...
exports.chunkNamePlugin = chunkNamePlugin;
const filePathMap = {};
const recordInfo = (id, chunkName, defaultChunkName, parentId) => {
    const sign = id.includes('node_modules') ? (0, build_utils_1.getPkgName)(id) : id;
...
const writeEmitter = new events_1.EventEmitter();
exports.writeEmitter = writeEmitter;
const fn = () => {
    const { writeDebounceTime } = (0, loadConfig_1.loadConfig)();
    return (0, cwd_1.debounce)(() => {
        if (hasWritten) {
...
const asyncOptimizeChunkPlugin = () => {
    return {
        ...
        transform(code, id) {
            moduleIds.push(id);
            (0, cwd_1.ssrDebug)(`build optimize process file ${id}`);
            checkBuildEnd();
        },
        async buildEnd(err) {
            ...
            Object.keys(dependenciesMap).forEach(item => {
                const id = !(0, path_1.isAbsolute)(item) ? filePathMap[item] : item;
                findChildren(id, this.getModuleInfo);
            });
            Object.keys(dependenciesMap).forEach(item => {
                if (!(0, path_1.isAbsolute)(item)) {
                    const abPath = filePathMap[item];
                    if (abPath) {
                        try {
                            const allDependencies = {};
                            // find absolute dependencies path from business file
                            (0, build_utils_1.getDependencies)(require.resolve(item, {
                                paths: [abPath]
                            }), allDependencies);
                            ...
                            (0, log_1.logErr)(`Please check ${(0, build_utils_1.getPkgName)(abPath)}/package.json ${abPath} use ${item} but don't specify it in dependencies`);
                        }
                    }
                }
            });
            ...
            return await new Promise((resolve) => {
                if (err) {
                    (0, log_1.logErr)(JSON.stringify(err));
...
exports.asyncOptimizeChunkPlugin = asyncOptimizeChunkPlugin;
const manifestPlugin = () => {
    const { getOutput, optimize } = (0, loadConfig_1.loadConfig)();
    ...
    return {
        ...
        async generateBundle(_, bundles) {
            ...
            for (const bundle in bundles) {
                ...
                arr.splice(1, 2);
                manifest[arr.join('.')] = `${(0, parse_1.getOutputPublicPath)()}${val}`;
            }
            if (!await (0, cwd_1.accessFile)((0, path_1.resolve)(clientOutPut))) {
                (0, shelljs_1.mkdir)('-p', (0, path_1.resolve)(clientOutPut));
            }
            manifest['vite'] = '1';
            await fs_1.promises.writeFile((0, path_1.resolve)(clientOutPut, './asset-manifest.json'), JSON.stringify(manifest, null, 2));
        }
    };
};
exports.manifestPlugin = manifestPlugin;
const writeGenerateMap = async () => {
    await fs_1.promises.writeFile((0, path_1.resolve)((0, cwd_1.getCwd)(), './build/asyncChunkMap.json'), JSON.stringify(asyncChunkMapJSON, null, 2));
    await fs_1.promises.writeFile((0, path_1.resolve)((0, cwd_1.getCwd)(), './build/generateMap.json'), JSON.stringify(generateMap, null, 2));
    await fs_1.promises.writeFile((0, path_1.resolve)((0, cwd_1.getCwd)(), './build/dependenciesMap.json'), JSON.stringify(dependenciesMap, null, 2));
};
...
const rollupOutputOptions = () => {
    const buildConfig = (0, build_config_1.getBuildConfig)();
    return {
...
const manualChunksFn = (id) => {
    ...
    if (!process.env.LEGACY_VITE) {
        ...
        if (arr.length === 1) {
            ...
            const commonChunkName = (0, cwd_1.cryptoAsyncChunkName)(arr.map(item => ({ name: item })), asyncChunkMapJSON);
...
const commonConfig = () => {
    ...
    const { whiteList, alias, css, hmr, viteConfig, optimize } = (0, loadConfig_1.loadConfig)();
    ...
    return {
        ...
        // @ts-expect-error
        ssr: {
            external: static_1.defaultExternal.concat((_o = (_m = (_l = viteConfig === null || viteConfig === void 0 ? void 0 : viteConfig()) === null || _l === void 0 ? void 0 : _l.server) === null || _m === void 0 ? void 0 : _m.externals) !== null && _o !== void 0 ? _o : []),
