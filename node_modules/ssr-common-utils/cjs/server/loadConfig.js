...
const loadConfig = () => {
    ...
    const mode = 'ssr';
    const stream = false;
    const isVite = (0, cwd_1.accessFileSync)((0, path_1.join)(cwd, './build/tag.json'));
    const optimize = process.env.OPTIMIZE === '1';
    const isCI = !!process.env.CI_TEST;
    const vue3ServerEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-vue3/esm/entry/server-entry.js');
    const vue3ClientEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-vue3/esm/entry/client-entry.js');
    const vueServerEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-vue/esm/entry/server-entry.js');
    const vueClientEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-vue/esm/entry/client-entry.js');
    const reactServerEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-react/esm/entry/server-entry.js');
    const reactClientEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-react/esm/entry/client-entry.js');
    const react18ServerEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-react18/esm/entry/server-entry.js');
    const react18ClientEntry = (0, path_1.join)(cwd, './node_modules/ssr-plugin-react18/esm/entry/client-entry.js');
    const supportOptinalChaining = (0, semver_1.coerce)(process.version).major >= 14;
    const define = (_a = userConfig.define) !== null && _a !== void 0 ? _a : {};
    userConfig.define && (0, cwd_1.stringifyDefine)(define);
    const alias = Object.assign({
        '@': (0, cwd_1.getFeDir)(),
        '~': (0, cwd_1.getCwd)(),
        '~/src': (0, path_1.join)(cwd, './src'),
        _build: (0, path_1.join)(cwd, './build')
    ...
    const publicPath = ((_b = userConfig.publicPath) === null || _b === void 0 ? void 0 : _b.startsWith('http')) ? userConfig.publicPath : (0, common_1.normalizeStartPath)((_c = userConfig.publicPath) !== null && _c !== void 0 ? _c : '/');
    ...
    const corejsVersion = (0, cwd_1.checkModuleExist)('core-js/package.json') ? (0, semver_1.coerce)(require((0, cwd_1.loadModuleFromFramework)('core-js/package.json')).version) : {};
    ...
    const manifestPath = `${(0, common_1.normalizeEndPath)(devPublicPath)}asset-manifest.json`;
    const staticPath = `${(0, common_1.normalizeEndPath)(devPublicPath)}${assetsDir}`;
    const hotUpdatePath = `${(0, common_1.normalizeEndPath)(devPublicPath)}*.hot-update**`;
    const proxyKey = [staticPath, hotUpdatePath, manifestPath];
    const prefix = '/';
    const dynamicFile = {
        serverBundle: (0, path_1.join)(cwd, `./build/server/${chunkName}.server.js`),
        assetManifest: (0, path_1.join)(cwd, './build/client/asset-manifest.json'),
        asyncChunkMap: (0, path_1.join)(cwd, './build/asyncChunkMap.json')
    };
    ...
    config.staticConfigPath = (0, path_1.join)(config.cwd, './build/staticConfig.js');
    config.getOutput = () => ({
        clientOutPut: (0, path_1.join)(cwd, './build/client'),
        serverOutPut: (0, path_1.join)(cwd, './build/server')
    });
    config.assetsDir = assetsDir;
    config.alias = alias;
    config.prefix = (0, common_1.normalizeStartPath)((_l = config.prefix) !== null && _l !== void 0 ? _l : '/');
