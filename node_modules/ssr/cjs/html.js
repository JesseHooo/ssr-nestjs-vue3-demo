...
const generateHtml = async () => {
    ...
    const { loadConfig, getCwd, judgeFramework, loadModuleFromFramework, logGreen, getAsyncJsChunk, logWarning, getAsyncCssChunk, splitPageInfo, getScriptArr, getStaticConfig } = await Promise.resolve().then(() => require('ssr-common-utils'));
  ...
  `;
    ...
    if (framework === 'ssr-plugin-vue3') {
        const { h, Fragment } = await Promise.resolve().then(() => require(loadModuleFromFramework('vue')));
        const { renderToString } = await Promise.resolve().then(() => require('@vue/server-renderer'));
    ...
    if (framework === 'ssr-plugin-vue') {
        const { h } = await Promise.resolve().then(() => require(loadModuleFromFramework('vue')));
        const Vue = await Promise.resolve().then(() => require(loadModuleFromFramework('vue')));
        const { createRenderer } = await Promise.resolve().then(() => require('vue-server-renderer'));
    ...
    if (framework === 'ssr-plugin-react' || framework === 'ssr-plugin-react18') {
        const { createElement: h, Fragment } = await Promise.resolve().then(() => require(loadModuleFromFramework('react')));
        const { renderToString } = await Promise.resolve().then(() => require(loadModuleFromFramework('react-dom/server')));
        for (const item of combine) {
            const { arr, flag } = item;
            const scriptArr = arr.map((item) => {
                var _a;
